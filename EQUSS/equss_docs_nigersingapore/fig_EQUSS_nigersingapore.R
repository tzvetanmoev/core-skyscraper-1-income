...
#for (i in 1:dim(full)[1]){
#  if (full$country[i] == "Singapore")
#    full$col[i] <- col_high
#  if (full$country[i] == "Niger")
#    full$col[i] <- col_low
#}
...
# add in transparency and change all other colours

for (i in 1:dim(final)[1]) {
  final$trans[i] <- 0
  if (final$country[i] == "Niger" | final$country[i] == "Singapore") {
    final$trans[i] <- 1
  }
}
...
sink(paste("fig_",k,".html", sep ="")) # comment out to restrict output
