...
for (i in 1:dim(full)[1]){
  if (full$country[i] != "Norway" | full$country[i] != "United States")
    full$population[i] <- full$population[i]
}
...
Prop_pop <- (sing_year$population/sum(sing_year$population))*50
...
for (i in 2:dim(sing_year.expanded)[1]) {
  if (sing_year.expanded$lab[i] == "United States" & i != 25) {
...
sink(paste("equss_docs_norwayus_popweight/fig_",k,".html", sep ="")) # comment out to restrict output
